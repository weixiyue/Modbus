...
from PySide2.QtWidgets import QApplication, QMessageBox, QTableWidgetItem, QVBoxLayout, QWidget, QLCDNumber
        ...
        if self.master.is_connect:
            ...
            if data == 0:
                ...
                if self.slave_running:
                    ...
                    # self.ui.StartCountLCD.display(self.slave_start_count)
            self.get_start_count(self.count_address)
            self.ui.StartCountLCD.display(self.slave_start_count)
    def get_start_count(self, count_address):
        response = self.master.execute(slave_id=self.slave_id, function_code=WRITE_SINGLE_COIL,
                                       starting_address=count_address)
        row, data = response
        return data
    def stop_slave(self):
        if self.master.is_connect:
